...
func main() {
	...
	http.HandleFunc("/crash", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "<h1>💥 Goodbye, World!</h1><p>Pod <b>%s</b> is crashing...</p>", podName)
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if data.LastError != nil {
			...
			fmt.Fprintf(w, "<a href='/crash' class='btn-crash'>💥 CRASH THIS POD</a>")
		}
...
func fetch(podName, ns string) WorkloadInfo {
	...
	info.Pods = []PodDetail{} // Initialize to prevent nil issues

	// 2. Traverse Owners
	for _, o := range p.OwnerReferences {
		if o.Name == "" { continue } // Safety check: skip if name is empty

		switch o.Kind {
		case "StatefulSet":
			ss, err := client.AppsV1().StatefulSets(ns).Get(context.Background(), o.Name, metav1.GetOptions{})
			if err == nil {
				selector = metav1.FormatLabelSelector(ss.Spec.Selector)
				info.Kind = "StatefulSet"
				info.KindName = o.Name
			}

		case "DaemonSet":
			ds, err := client.AppsV1().DaemonSets(ns).Get(context.Background(), o.Name, metav1.GetOptions{})
			if err == nil {
		...
		case "ReplicaSet":
			rs, err := client.AppsV1().ReplicaSets(ns).Get(context.Background(), o.Name, metav1.GetOptions{})
			if err == nil {
				// Check if this RS is owned by a Deployment
				isDeployment := false
				for _, ro := range rs.OwnerReferences {
					if ro.Kind == "Deployment" && ro.Name != "" {
						info.Kind = "Deployment"
						info.KindName = ro.Name
						info.RSName = rs.Name
				...
				// If it's just a standalone ReplicaSet
				if !isDeployment {
					info.Kind = "ReplicaSet"
					info.KindName = rs.Name
					selector = metav1.FormatLabelSelector(rs.Spec.Selector)
				}
			}
		}
	...
	// 3. Fetch Peers if we found a selector
	if selector != "" {
		podList, err := client.CoreV1().Pods(ns).List(context.Background(), metav1.ListOptions{LabelSelector: selector})
		if err == nil {
		...
		// Fallback for standalone pods
		info.Kind = "Standalone Pod"
		info.KindName = podName
		info.Pods = append(info.Pods, PodDetail{Name: podName, Node: p.Spec.NodeName})
	}
